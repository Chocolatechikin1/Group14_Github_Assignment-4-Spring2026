...
import {
  CAL_EVENTS, CalEvent, COURSES, ExtraBlock, extraBlockToCalEvent,
...
export default function CalendarScreen({ extraBlocks = [] }: Props) {
  const [selectedDay, setSelectedDay]     = useState(0); // 0 = Mon
  const [selectedEvent, setSelectedEvent] = useState<CalEvent | null>(null);
  const [showConflict, setShowConflict]   = useState(false);

  // Merge user-created study blocks into the event list so they show up
  // on the calendar grid alongside scheduled course events.
  const allEvents     = [
    ...CAL_EVENTS,
    ...extraBlocks.map(extraBlockToCalEvent),
  ];
  const isConflictDay = selectedDay === 2; // Wednesday
  const dayEvents     = allEvents.filter(e => e.day === selectedDay + 1);
    ...
    <SafeAreaView style={shared.screen}>
      ...
      >
        {WEEK_DAYS.map((day, i) => {
          ...
          const hasEvents  = allEvents.some(e => e.day === i + 1);
